...
class CanvasImage:
    ...
    def place(self, **kw):
        ...
        raise Exception('Cannot use place with this widget ' + self.__class__.__name__)
...
class ImageResizingSection:
    def __init__(self, parent, get_loaded_image_paths, display_images, set_images_path):
        self.parent = parent
        self.get_loaded_image_paths = get_loaded_image_paths
        self.display_images = display_images
        self.set_images_path = set_images_path
    ...
    def resize_images(self):
        ...
        loaded_image_paths = self.get_loaded_image_paths()
        if not loaded_image_paths:
            messagebox.showwarning("Warning", "Please load images or extract frames first.")
            return

        try:
            resized_image_paths = process_images(loaded_image_paths, scale_percent)
            self.set_images_path(os.path.dirname(resized_image_paths[0]))  # Mettre à jour le chemin des images ici
...
class BackgroundRemovalSection:
    def __init__(self, parent, get_loaded_image_paths, get_input_images_folder, display_images):
        self.parent = parent
        self.get_loaded_image_paths = get_loaded_image_paths
        self.get_input_images_folder = get_input_images_folder
        self.display_images = display_images
    ...
    def remove_background(self):
        ...
        try:
            input_folder = self.get_input_images_folder()
            output_folder = os.path.dirname(input_folder)

            if self.bg_removal_algorithm == "rembg":
                remove_background_rembg(input_folder, output_folder)
            else:
                remove_background_briarmbg(input_folder, output_folder)

            self.display_images(extract_images(os.path.join(output_folder, 'images')))
...
class PhotogrammetrySection:
    ...
    def configure_photogrammetry_section(self):
        ...
        photogrammetry_title = ctk.CTkLabel(photogrammetry_frame, text="Photogrammetry", font=("Arial", 14))
        photogrammetry_title.pack(pady=5)

        radio_point_cloud = ctk.CTkRadioButton(photogrammetry_frame, text="Point Cloud", variable=self.processing_option, value="point_cloud")
        radio_point_cloud.pack(pady=5)

        radio_3d_model = ctk.CTkRadioButton(photogrammetry_frame, text="3D Model", variable=self.processing_option, value="3d_model")
        radio_3d_model.pack(pady=5)

        photogrammetry_process_button = ctk.CTkButton(photogrammetry_frame, text="Run Photogrammetry", command=self.run_photogrammetry)
        photogrammetry_process_button.pack(pady=5)

    def run_script(self, script_name, project_path, folder_name):
    ...
    def run_photogrammetry(self):
        try:
            ...
            print(f"Script Path: {os.path.join(os.path.dirname(__file__), 'colmap_demo.sh')}")
            print(f"Command: {os.path.join(os.path.dirname(__file__), 'colmap_demo.sh')} {project_path} {folder_name}")
            print(f"Images Folder : {images_path}")
            print(f"Project Path: {project_path}")
            print(f"Folder Name: {folder_name}")

            if self.processing_option.get() == "point_cloud":
                self.run_script("colmap_demo.sh", project_path, folder_name)
            else:
                self.run_script("colmap_openmvs.sh", project_path, folder_name)
        except Exception as e:
            messagebox.showerror("Error", f"An error occurred: {e}")
...
class SidebarSection:
    ...
    def configure_sidebar_section(self):
        ...
        self.sidebar_title_frame.pack(fill="x", pady=5)
...
class PhotogrammetryPage(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.image_resizing_section = ImageResizingSection(self.sidebar_section.sidebar_frame, self.get_loaded_image_paths, self.display_images, self.set_images_path)
        self.background_removal_section = BackgroundRemovalSection(self.sidebar_section.sidebar_frame, self.get_loaded_image_paths, self.get_input_images_folder, self.display_images)
