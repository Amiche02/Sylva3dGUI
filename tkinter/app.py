...
class HomePage(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        # Title
        title = ctk.CTkLabel(self.scrollable_frame, text="Welcome to Sylva3D Photogrammetry Application", font=("Arial", 20))
        title.pack(pady=20)
        ...
        overview = ctk.CTkLabel(self.scrollable_frame, text="Sylva3D Photogrammetry is a powerful tool for generating 3D models from 2D images. "
                                ...
                                font=("Arial", 14), wraplength=1000, justify="left")
        ...
        # LinkedIn Link
        linkedin_link = ctk.CTkLabel(self.scrollable_frame, text="linkedin.com/in/stephanekpoviessi/", font=("Arial", 14), fg_color="blue", cursor="hand2")
        linkedin_link.pack(pady=10)
        linkedin_link.bind("<Button-1>", lambda e: webbrowser.open("https://linkedin.com/in/stephanekpoviessi/"))

        # Message Section
        message_label = ctk.CTkLabel(self.scrollable_frame, text="Send me a message:", font=("Arial", 16))
        message_label.pack(pady=20)

        self.message_entry = ctk.CTkEntry(self.scrollable_frame, width=800, height=100, font=("Arial", 14))
        self.message_entry.pack(pady=10)

        send_button = ctk.CTkButton(self.scrollable_frame, text="Send Message", command=self.send_message, font=("Arial", 14), width=200, height=50)
        send_button.pack(pady=10)

    def send_message(self):
        message = self.message_entry.get()
        if message:
            # Implement the logic to send the message via email or other means
            messagebox = ctk.CTkMessagebox(self, title="Message Sent", message="Your message has been sent successfully!")
            messagebox.open()
        else:
            messagebox = ctk.CTkMessagebox(self, title="Empty Message", message="Please enter a message before sending!")
            messagebox.open()
